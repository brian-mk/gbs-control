...
// BAK 29/11/2022
// Modifed to support Marconi 2380 Spectrum Analyser RGBS display (h:15.625kHz 64uS/line v:48.2Hz 324 lines, non-interlaced)
// Display geometry: Graticule:250 lines leaving 74 lines (37 lines top margin, 37 lines bottom margin)
// Characters use a 7 x 12 dot cell. Total cells = 66 x 24. Total dots = 462 x 288. Aspect ratio: 1.6:1 or 8:5 (assuming square dots).
// Hence effective horizontal pixels = 324 * 8/5 = 518.
// Graticule top & bottom margins can hold 2 lines of characters each. i.e. 24 lines + 13 blank. Left & Right margins hold 8 characters each.
//
// Note: The 5725 'Mode Detect Status' VPERIOD_IF (S0_07 & S0_08) reports 324 line input as 647. i.e. double the expected value.
// HPERIOD_IF (S0_06 & S0_7) is reported as 430 (= <total pixels> / 4). Hence H total pixels = 1720.
// STATUS_SYNC_PROC_HTOTAL (S0_17) reports no of pixels as 2269.
// STATUS_SYNC_PROC_VTOTAL (S0_1B) reports no of lines as 323 when in sync. When out of sync, this can vary. Hence it cannot
// be reliably used to detect 324 lines.
// All the above registers are read-only.
//
// 324 lines is handled as follows:-
// A new videoMode added: VM_PAL_324p (10). This is handled in much the same way as the existing VM_PAL_576p (50Hz).
// The ASIC Mode Detect section cannot auto-detect 15.625kHz inputs other than NTSC and PAL.
// Note: The 'user defined resolution' mode detection (S0_05), MD_USER_DEF_VCNTRL (S1_80), MD_USER_DEF_HCNTRL (S1_81)
// cannot be used for 15.625kHz line rate because MD_USER_DEF_HCNTRL is only 8 bits. The 5725 documentation
// indicates it requires a value of 6750000/15625 = 432 (0x1B0).
//
// In getVideomode(), if STATUS_00 indicates input h & v timing stable, then STATUS_SYNC_PROC_VTOTAL is read.
// This is read another two times in succession.
// If the additional two reads indicate the same no of lines (+/- 1), notRecognizedCounter is incremented and VM_UNKNOWN is returned.
// After further calls to getVideoMode() if notRecognizedCounter reaches 255, the returned videoMode is changed from VM_UKNOWN to VM_NOT_RECOGNISED.
// An attempt to use the above mechanism to detect 324 lines failed because although STATUS_SYNC_PROC_VTOTAL indicates 323 lines
// when in sync, the value can be unstable when out of sync.
// Instead, if STATUS_00 indicates input h & v timing stable, VPERIOD_IF is compared with both 324 +/- VPERIOD_TOLERANCE and (324 * 2) +/- VPERIOD_TOLERANCE.
//
// Replaced many 'constants in code' with enums for better readability.
...
// presetID
typedef enum presetID_e {
  PID_1280x960_NTSC  = 0x01,
  PID_1280x960_PAL   = 0x11,
  PID_1280x1024_NTSC = 0x02,
  PID_1280x1024_PAL  = 0x12,
  PID_1280x720_NTSC  = 0x03,
  PID_1280x720_PAL   = 0x13,
  PID_720x480        = 0x04,
  PID_768x576        = 0x14,
  PID_1920x1080_NTSC = 0x05,
  PID_1920x1080_PAL  = 0x15,  
  PID_DOWNSCALE_NTSC = 0x06,
  PID_DOWNSCALE_PAL  = 0x16,
  PID_CUSTOM         = 0x09,
  PID_BYPASS_1       = 0x21,
  PID_BYPASS_2       = 0x22
} presetID_t;
...
// preset preferences
typedef enum preset_e {
  PP_NORMAL,            // 0
  PP_720x480_768x576,   // 1 4:3
  PP_CUSTOM,            // 2
  PP_1280x720,          // 3 16:9
  PP_1280x1024,         // 4 5:4
  PP_1920x1080,         // 5 16:9
...
typedef enum vm_e {
  ...
  VM_NTSC_480p,         // 3 4:3
  VM_PAL_576p,          // 4 4:3
  VM_HD_720p,           // 5 1280x720  16:9
  VM_HD_1080i,          // 6 1920x1080 16:9
  VM_HD_1080p,          // 7 1920x1080 16:9
  VM_MED_RES_25kHz,     // 8
  VM_NOT_RECOGNISED,    // 9
  VM_PAL_324p,          // 10 4:3 For Marconi 2380
  VM_11,
  VM_12,
  VM_VGA_SVGA_XGA_SXGA, // 13
  VM_VGA_UPSCALE,       // 14
  VM_RGBHV              // 15
} vm_t;

// sync input
typedef enum sync_in_e {
  SI_UNKNOWN,           // 0
  SI_RGBS,              // 1
  SI_YpBpR,             // 2
  SI_RGBHV              // 3
...
static vm_t getVideoMode(void);
static vm_t applyPresets(vm_t videoMode);
static boolean videoModeIsPalNtscSd(vm_t videoMode);
static boolean videoModeIsPalNtsc(vm_t videoMode);
static boolean videoModeIsPal(vm_t videoMode);
static boolean videoModeIsNtsc(vm_t videoMode);
static const uint8_t *loadPresetFromSPIFFS(vm_t forVideoMode);
static sync_in_t detectAndSwitchToActiveInput(void);
static sync_in_t inputAndSyncDetect(void);
static void resolution_OLED( vm_t videoMode, preset_t presetPreference, const String& s);
static void settingsMenuOLED(void);
...
struct runTimeOptions
{
    ...
    vm_t videoStandardInput; // 0 - unknown, 1 - NTSC like, 2 - PAL like, 3 480p NTSC, 4 576p PAL
    uint8_t phaseSP;
    uint8_t phaseADC;
...
#define PLL648_CONTROL_01_EXTCLK 0x75 // Select Ext clock on PCLKIN (pin 40)
#define PLL648_CONTROL_01_ADCCLK 0x35 // Select Int clock from ADC
...
void externalClockGenResetClock()
{
    ...
    else if (activeDisplayClock == PLL648_CONTROL_01_ADCCLK)
...
void externalClockGenSyncInOutRate()
{
    ...
    if (GBS::PLL648_CONTROL_01::read() != PLL648_CONTROL_01_EXTCLK) {
        return;
    }
...
static boolean videoModeIsPalNtscSd(vm_t videoMode)
{
    if ((videoMode == VM_NTSC_480i) || (videoMode == VM_PAL_576i)) {
        return true;
    }
    return false;
}
...
{
    if ((videoMode == VM_NTSC_480i) || (videoMode == VM_PAL_576i) ||
        (videoMode == VM_NTSC_480p) || (videoMode == VM_PAL_576p) || (videoMode == VM_PAL_324p)) {
        return true;
    }
    return false;
}
...
static boolean videoModeIsPal(vm_t videoMode)
{
    if ((videoMode == VM_PAL_576i) || (videoMode == VM_PAL_576p)|| (videoMode == VM_PAL_324p)) {
        return true;
    }
    return false;
}
...
static boolean videoModeIsNtsc(vm_t videoMode)
{
    if ((videoMode == VM_NTSC_480i) || (videoMode == VM_NTSC_480p) || (videoMode == VM_PAL_324p)) {
        return true;
    }
    return false;
}
...
void writeProgramArrayNew(const uint8_t *programArray, boolean skipMDSection)
{
    ...
    if (rto->videoStandardInput == VM_RGBHV) {
        rto->videoStandardInput = VM_UNKNOWN;
    }
            ...
            case 5:
                for (int j = 0; j <= 6; j++) { // 7 times
                    for (int x = 0; x <= 15; x++) {
                        ...
                        if (index == 375) { // s5_37
                            if (videoModeIsPalNtscSd(rto->videoStandardInput)) {
                                bank[x] = 0x6b;
                            } else {
                                bank[x] = 0x02;
                            }
    ...
    if (uopt->preferScalingRgbhv && rto->isValidForScalingRGBHV) {
        ...
        rto->videoStandardInput = VM_NTSC_480p;
    }
}
...
void activeFrameTimeLockInitialSteps()
{
    ...
    if (rto->presetID != PID_BYPASS_1 && rto->presetID != PID_BYPASS_2) {
...
void applyYuvPatches()
{
    ...
    if (GBS::GBS_PRESET_CUSTOM::read() == 0) {
        ...
        if ((rto->videoStandardInput == VM_HD_720p) || (rto->videoStandardInput == VM_HD_1080i) || (rto->videoStandardInput == VM_HD_1080p)) {
...
void prepareSyncProcessor()
{
    ...
    if (rto->videoStandardInput == VM_UNKNOWN)
        GBS::SP_DLT_REG::write(0x70); // 5_35  0x70 = 112d. 130 too much for ps2 1080i, 0xb0 for 1080p    
    else if (videoModeIsPalNtsc(rto->videoStandardInput)) // includes VM_PAL_324p
        GBS::SP_DLT_REG::write(0xC0); // 0xC0 = 192d. old: extended to 0x150 later if mode = 1 or 2
    else if ((rto->videoStandardInput == VM_HD_720p) || (rto->videoStandardInput == VM_HD_1080i))
        GBS::SP_DLT_REG::write(0xA0);  // 0xA0 = 160
    else if (rto->videoStandardInput == VM_HD_1080p)
        GBS::SP_DLT_REG::write(0x70);
    else
        // VM_MED_RES || VM_VGA_SVGA_XGA_SXGA || VM_VGA_UPSCALE || VM_RGBHV
        GBS::SP_DLT_REG::write(0x70);
    ...
    if ((rto->videoStandardInput != VM_RGBHV) && (GBS::GBS_OPTION_SCALING_RGBHV::read() != 1)) {
...
void goLowPowerWithInputDetection()
{
    ...
    //zeroAll();
    setResetParameters(); // includes rto->videoStandardInput = VM_UNKNOWN
...
boolean optimizePhaseSP()
{
    ...
    if (rto->currentLevelSOG <= 2) {
        ...
        rto->phaseSP = 16;
        rto->phaseADC = 16;
        if (videoModeIsPalNtsc(rto->videoStandardInput)) {
            if (rto->osr == 4) {
                rto->phaseADC += 16;
                rto->phaseADC &= 0x1f;
            }
    ...
    if (runTest) {
        ...
        if (worstBadHt != 0) {
            ...
            } else if (videoModeIsPalNtsc(rto->videoStandardInput)) {
                if (rto->osr == 4) {
                    //Serial.println("shift adc phase");
                    rto->phaseADC += 16;
                    rto->phaseADC &= 0x1f;
...
void optimizeSogLevel()
{
    ...
    if ((rto->videoStandardInput == VM_RGBHV) || GBS::SP_SOG_MODE::read() != 1 || rto->syncTypeCsync == false) {
        rto->thisSourceMaxLevelSOG = rto->currentLevelSOG = 13;
        return;
    }
...
{ // if any
    ...
    while (millis() - timeout < 450) {
            ...
            if (currentInput == 1) { // RGBS or RGBHV
                ...
                if (vsyncActive) {
                    ...
                    if (hsyncActive) {
                        ...
                        for (uint8_t i = 0; i < 16; i++) {
                            //printInfo();
                            vm_t innerVideoMode = getVideoMode();
                            if (innerVideoMode == VM_MED_RES_25kHz) {
                                setAndUpdateSogLevel(rto->currentLevelSOG);
                        ...
                        rto->videoStandardInput = applyPresets(VM_RGBHV);
                        delay(100);
                ...
                if (!vsyncActive) { // then do RGBS check
                    ...
                    while ((millis() - timeOutStart) < 6000) {
                        delay(2);
                        if (getVideoMode() != VM_UNKNOWN) {
                            if (getVideoMode() != VM_MED_RES_25kHz) { // if it's mode 8, need to set stuff first
                                return SI_RGBS;
                            }
                        }
                        testCycle++;
                        ...
                        if (getVideoMode() == VM_MED_RES_25kHz) {
                        ...
                        if (currentMedResLineCount < 0x3c) {                            // 0x3c=60d. 60 * 8 = 480
                            GBS::MD_HD1250P_CNTRL::write(currentMedResLineCount + 1);
                        } else {
                            GBS::MD_HD1250P_CNTRL::write(0x33);                         // 0x33=51d. 51 * 8 = 408
                        }
                        //Serial.println(GBS::MD_HD1250P_CNTRL::read(), HEX);
                    }
                    ...
                    return SI_RGBS; //anyway, let later stage deal with it
                }
                ...
                return SI_YpBpR; //anyway, let later stage deal with it
            }
        ...
        return SI_UNKNOWN; // don't do the check on the new input here, wait till next run
    }
...
static sync_in_t inputAndSyncDetect(void)
{
    sync_in_t syncInput = detectAndSwitchToActiveInput();

    if (syncInput == SI_UNKNOWN) {
        if (!getSyncPresent()) {
        ...
        return SI_UNKNOWN;
    } else if (syncInput == SI_RGBS) { // input is RGBS
        rto->inputIsYpBpR = 0;
        ...
        LEDON;
        return SI_YpBpR;
    } else if (syncInput == SI_RGBHV) { // input is RGBHV
        //already applied
        rto->isInLowPowerMode = false;
        rto->inputIsYpBpR = 0;
        rto->sourceDisconnected = false;
        rto->videoStandardInput = VM_RGBHV;
        resetDebugPort();
        LEDON;
        return SI_RGBHV;
    }
...
void shiftHorizontalLeftIF(uint8_t amount)
{
    ...
    } else if ((rto->videoStandardInput == VM_NTSC_480p) || (rto->videoStandardInput == VM_PAL_576p) || (rto->videoStandardInput == VM_PAL_324p) ||
               (rto->videoStandardInput == VM_HD_720p) || (rto->videoStandardInput == VM_HD_1080i) || (rto->videoStandardInput == VM_HD_1080p)) {
...
void shiftVerticalUpIF()
{
    ...
    uint8_t offset = rto->videoStandardInput == VM_PAL_576i ? 4 : 1;
    ...
    if ((GBS::GBS_OPTION_SCALING_RGBHV::read() == 1) && rto->videoStandardInput == VM_VGA_UPSCALE) {
...
{
    uint16_t vds_dis_vb_st = vtotal - 2;                         // Final display v blanking start. just below vtotal
    uint16_t vds_dis_vb_sp = (vtotal >> 6) + 8;                  // Final display v blanking stop. positive, above new sync stop position
    ...
    if ((vtotal < 530) || (vtotal >= 803 && vtotal <= 809) || (vtotal >= 793 && vtotal <= 798)) {
        // swap v sync start & stop positions
        uint16_t temp = v_sync_start_position;
        v_sync_start_position = v_sync_stop_position;
        v_sync_stop_position = temp;
    }

    GBS::VDS_VSYNC_RST::write(vtotal);            // Internal vertical period control
    GBS::VDS_VS_ST::write(v_sync_start_position); // v sync start
    GBS::VDS_VS_SP::write(v_sync_stop_position);  // v sync stop
    GBS::VDS_VB_ST::write(vds_vb_st);             // v blanking start
    GBS::VDS_VB_SP::write(vds_vb_sp);             // v blanking stop
    GBS::VDS_DIS_VB_ST::write(vds_dis_vb_st);     // Final display v blanking start
    GBS::VDS_DIS_VB_SP::write(vds_dis_vb_sp);     // Final display v blanking stop
...
void setIfHblankParameters()
{
    if (!rto->outModeHdBypass) {
        ...
        if (rto->presetID == PID_1920x1080_NTSC) {
        ...
        if (rto->presetID == PID_1920x1080_PAL) {
        ...
        if (GBS::IF_LD_RAM_BYPS::read()) {
            ...
            if (rto->presetID == PID_1920x1080_NTSC || rto->presetID == PID_1920x1080_PAL) {
...
{
    if (!FrameSync::ready() && rto->autoBestHtotalEnabled == true && (rto->videoStandardInput != VM_UNKNOWN) && (rto->videoStandardInput != VM_RGBHV)) {
...
boolean applyBestHTotal(uint16_t bestHTotal)
{
    ...
    if (isLargeDiff && (getVideoMode() == VM_MED_RES_25kHz || rto->videoStandardInput == VM_VGA_UPSCALE)) {
    ...
    if (isLargeDiff && (rto->forceRetime == false)) {
        if (rto->videoStandardInput != VM_VGA_UPSCALE) {
...
void doPostPresetLoadSteps()
{
    ...
    if (rto->videoStandardInput == VM_UNKNOWN) {
        vm_t videoMode = getVideoMode();
        SerialM.print(F("post preset: rto->videoStandardInput 0 > "));
        SerialM.println((uint8_t)videoMode);
        if (videoMode != VM_UNKNOWN) {
    ...
    if (rto->presetID == PID_DOWNSCALE_NTSC || rto->presetID == PID_DOWNSCALE_PAL) {
        isCustomPreset = 0; // override so it applies section 2 deinterlacer settings
    }

    if (!isCustomPreset) {
        if (rto->videoStandardInput == VM_NTSC_480p || rto->videoStandardInput == VM_PAL_576p || (rto->videoStandardInput == VM_PAL_324p) ||
            rto->videoStandardInput == VM_HD_1080p || rto->videoStandardInput == VM_MED_RES_25kHz) {
        ...
        if (uopt->wantFullHeight) {
            if (videoModeIsNtsc(rto->videoStandardInput)) {
                //if (rto->presetID == PID_1920x1080_NTSC)
                //{ // out 1080p 60
                ...
                //}
            } else if (videoModeIsPal(rto->videoStandardInput)) {
                if (rto->presetID == PID_1920x1080_PAL) { // out 1080p 50
                    GBS::VDS_VSCALE::write(455);
        ...
        if (videoModeIsNtsc(rto->videoStandardInput)) {
            ...
            if (rto->presetID == PID_DOWNSCALE_NTSC || rto->presetID == PID_DOWNSCALE_PAL) {
                ...
                if (rto->videoStandardInput == VM_NTSC_480i) {
                ...
                } else if (rto->videoStandardInput == VM_PAL_576i) {
        ...
        if (rto->videoStandardInput == VM_NTSC_480p || rto->videoStandardInput == VM_PAL_576p || (rto->videoStandardInput == VM_PAL_324p) ||
            rto->videoStandardInput == VM_MED_RES_25kHz || rto->videoStandardInput == VM_NOT_RECOGNISED) {
            ...
            if (rto->videoStandardInput == VM_NOT_RECOGNISED) {
        ...
        if (rto->videoStandardInput == VM_NTSC_480p && rto->presetID != PID_DOWNSCALE_NTSC) { // ED YUV 60
            setCsVsStart(16);                                        // ntsc
            setCsVsStop(13);                                         //
            GBS::IF_HB_ST::write(30);                                // 1_10; magic number
            GBS::IF_HBIN_ST::write(0x20);                            // 1_24
            GBS::IF_HBIN_SP::write(0x60);                            // 1_26
            if (rto->presetID == PID_1920x1080_NTSC) {               // out 1080p
                GBS::IF_HB_SP2::write(0xB0);                         // 1_1a
                GBS::IF_HB_ST2::write(0x4BC);                        // 1_18
            } else if (rto->presetID == PID_1280x720_NTSC) {         // out 720p
                GBS::VDS_VSCALE::write(683);                         // same as base preset
                GBS::IF_HB_ST2::write(0x478);                        // 1_18
                GBS::IF_HB_SP2::write(0x84);                         // 1_1a
            } else if (rto->presetID == PID_1280x1024_NTSC) {           // out x1024
                GBS::IF_HB_SP2::write(0x84);                         // 1_1a
                GBS::IF_HB_ST2::write(0x478);                        // 1_18
            } else if (rto->presetID == PID_1280x960_NTSC) {         // out x960
                GBS::IF_HB_SP2::write(0x84);                         // 1_1a
                GBS::IF_HB_ST2::write(0x478);                        // 1_18
            } else if (rto->presetID == PID_720x480) {               // out x480
                GBS::IF_HB_ST2::write(0x478);                        // 1_18
                GBS::IF_HB_SP2::write(0x90);                         // 1_1a
            }
        } else if ( (rto->videoStandardInput == VM_PAL_576p) && rto->presetID != PID_DOWNSCALE_PAL) { // ED YUV 50
            ...
            if (rto->presetID == PID_1920x1080_PAL) {                       // out 1080p
                GBS::IF_HB_ST2::write(0x4C0);                               // 1_18
                GBS::IF_HB_SP2::write(0xC8);                                // 1_1a
            } else if (rto->presetID == PID_1280x720_PAL) {                 // out 720p
                GBS::IF_HB_ST2::write(0x478);                               // 1_18
                GBS::IF_HB_SP2::write(0x88);                                // 1_1a
            } else if (rto->presetID == PID_1280x1024_PAL) {                // out x1024
                ...
                GBS::IF_HB_SP2::write(0x88);    // 1_1a
            } else if (rto->presetID == PID_1280x960_PAL) { // out x960
                GBS::IF_HB_ST2::write(0x454);   // 1_18
                GBS::IF_HB_SP2::write(0x88);    // 1_1a
            } else if (rto->presetID == PID_768x576) { // out x576
                GBS::IF_HB_ST2::write(0x478);   // 1_18
                GBS::IF_HB_SP2::write(0x90);    // 1_1a
            }
        } else if (rto->videoStandardInput == VM_HD_720p) { // 720p
        ...
        } else if (rto->videoStandardInput == VM_HD_1080i || rto->videoStandardInput == VM_HD_1080p) { // 1080i/p
            GBS::ADC_FLTR::write(1);                                               // 5_03
            GBS::PLLAD_KS::write(0);                                               // 5_16
            ...
            GBS::INPUT_FORMATTER_02::write(0x74);
            GBS::VDS_Y_DELAY::write(3);
        } else if (rto->videoStandardInput == VM_MED_RES_25kHz) { // 25khz
            ...
            if (rto->presetID == PID_1280x960_NTSC) {   // out x960
                GBS::VDS_VSCALE::write(410);
            } else if (rto->presetID == PID_1280x1024_NTSC) { // out x1024
                GBS::VDS_VSCALE::write(402);
            } else if (rto->presetID == PID_1280x720_NTSC) { // out 720p
                GBS::VDS_VSCALE::write(546);
            } else if (rto->presetID == PID_1920x1080_NTSC) { // out 1080p
    ...
    if (isCustomPreset) {
        ...
        if (rto->videoStandardInput == VM_NTSC_480p || rto->videoStandardInput == VM_PAL_576p ||
           (rto->videoStandardInput == VM_PAL_324p) || rto->videoStandardInput == VM_MED_RES_25kHz) {
            GBS::MADPT_Y_DELAY_UV_DELAY::write(1); // 2_17 : 1
        }
        ...
        if (GBS::PLL648_CONTROL_01::read() == PLL648_CONTROL_01_EXTCLK && GBS::GBS_PRESET_DISPLAY_CLOCK::read() != 0) {
    ...
    if (!rto->outModeHdBypass && rto->autoBestHtotalEnabled &&
        ...
        (videoModeIsPalNtsc(rto->videoStandardInput))) {
        // autobesthtotal
    ...
    if (!isCustomPreset) {
        if (videoModeIsPalNtscSd(rto->videoStandardInput) && !rto->outModeHdBypass) {
    ...
    if (!isCustomPreset) {
        ...
        if ( (rto->videoStandardInput == VM_NTSC_480p) || (rto->videoStandardInput == VM_PAL_576p) ||
             (rto->videoStandardInput == VM_PAL_324p) ) {
    ...
    GBS::SP_H_PROTECT::write(0);
    if (!((rto->videoStandardInput == VM_UNKNOWN) || videoModeIsPalNtsc(rto->videoStandardInput))) {
    ...
    if (GBS::GBS_OPTION_SCALING_RGBHV::read() == 1) {
        rto->videoStandardInput = VM_VGA_UPSCALE;
    }
    ...
    if (GBS::GBS_OPTION_SCALING_RGBHV::read() == 0) {
        ...
        while ((getVideoMode() == VM_UNKNOWN) && (millis() - timeout < 1505)) {
            delay(4);
            handleWiFi(0);
            updateSpDynamic(0);
        }
    ...
    // also need to check for mode VM_RGBHV (15)
    // also make sure to turn off autoBestHtotal
    if (uopt->presetPreference == PP_BYPASS && rto->videoStandardInput != VM_RGBHV) {
    ...
    SerialM.print(F("\npreset applied: "));
    if (rto->presetID == PID_1280x960_NTSC || rto->presetID == PID_1280x960_NTSC)
        SerialM.print(F("1280x960"));
    else if (rto->presetID == PID_1280x1024_NTSC || rto->presetID == PID_1280x1024_PAL)
        SerialM.print(F("1280x1024"));
    else if (rto->presetID == PID_1280x720_NTSC || rto->presetID == PID_1280x720_PAL)
        SerialM.print(F("1280x720"));
    else if (rto->presetID == PID_1920x1080_NTSC || rto->presetID == PID_1920x1080_PAL)
        SerialM.print(F("1920x1080"));
    else if (rto->presetID == PID_DOWNSCALE_NTSC || rto->presetID == PID_DOWNSCALE_PAL)
        SerialM.print(F("downscale"));
    else if (rto->presetID == PID_720x480)
        SerialM.print(F("720x480"));
    else if (rto->presetID == PID_768x576)
        SerialM.print(F("768x576"));
    else
        SerialM.print(F("bypass"));

    if (isCustomPreset) {
        rto->presetID = PID_CUSTOM; // overwrite to "custom" after printing original id (for webui)
    }
    if (rto->outModeHdBypass) {
        SerialM.print(F(" (bypass)"));
    ...
    else if (rto->videoStandardInput == VM_NTSC_480p)
        SerialM.print(F("480p 60Hz "));
    else if (rto->videoStandardInput == VM_PAL_576p)
        SerialM.print(F("576p 50Hz "));
    else if (rto->videoStandardInput == VM_HD_720p)
        SerialM.print(F("720p 60Hz "));
    else if (rto->videoStandardInput == VM_HD_1080i)
        SerialM.print(F("1080i 60Hz "));
    else if (rto->videoStandardInput == VM_HD_1080p)
        SerialM.print(F("1080p 60Hz "));
    else if (rto->videoStandardInput == VM_MED_RES_25kHz)
        SerialM.print(F("Medium Res "));
    else if (rto->videoStandardInput == VM_PAL_324p)
        SerialM.print(F("324p 50Hz"));
    else if (rto->videoStandardInput == VM_VGA_SVGA_XGA_SXGA)
        SerialM.print(F("VGA/SVGA/XGA/SXGA"));
    else if (rto->videoStandardInput == VM_VGA_UPSCALE) {
        if (rto->syncTypeCsync)
            SerialM.print(F("scaling RGB (CSync)"));
        else
            SerialM.print(F("scaling RGB (HV Sync)"));
    } else if (rto->videoStandardInput == VM_RGBHV) {
        if (rto->syncTypeCsync)
            SerialM.print(F("RGB Bypass (CSync)"));
        else
            SerialM.print(F("RGB Bypass (HV Sync)"));
    } else if (rto->videoStandardInput == VM_UNKNOWN)
        SerialM.print(F("Unknown video mode !should not go here!"));
    ...
    // presetPreference = PP_CUSTOM (2) may fail to load (missing) preset file and arrive here with defaults
    SerialM.println("\n");
}
...
static vm_t applyPresets(vm_t videoMode)
{
    ...
    boolean waitExtra = 0;
    if (rto->outModeHdBypass || rto->videoStandardInput == VM_RGBHV || rto->videoStandardInput == VM_UNKNOWN) {
        waitExtra = 1;
        if (videoMode == VM_UNKNOWN || videoModeIsPalNtsc(videoMode) ||
            videoMode == VM_VGA_UPSCALE || videoMode == VM_MED_RES_25kHz || videoMode == VM_NOT_RECOGNISED) {
            GBS::SFTRST_IF_RSTZ::write(1); // take formatter out of reset. early init
            GBS::SFTRST_VDS_RSTZ::write(1); // take video display proc out of reset
            GBS::SFTRST_DEC_RSTZ::write(1); // take decimation out of reset
        }
    }
    ...
    if (GBS::ADC_UNUSED_62::read() != 0x00) {
        ...
        if (uopt->presetPreference != PP_CUSTOM) {
            typeOneCommand = 'D';
        }
    }
    ...
    if (videoMode == VM_UNKNOWN) {
        ...
        videoMode = VM_NTSC_480p;                   // in case of success: override to 480p60
        ...
        if (GBS::STATUS_SYNC_PROC_HSACT::read() == 1) {
            ...
            if (GBS::STATUS_SYNC_PROC_HSACT::read() == 1) {
                ...
                rto->videoStandardInput = VM_NTSC_480p; // override to 480p60
                doPostPresetLoadSteps();
                GBS::SP_CLAMP_MANUAL::write(1);
                rto->syncWatcherEnabled = 0;
                videoMode = VM_UNKNOWN;
    ...
    if (uopt->PalForce60 == 1) {
        if (uopt->presetPreference != PP_CUSTOM) { // != custom. custom saved as pal preset has ntsc customization
            if (videoModeIsPal(videoMode)) {
                Serial.println(F("PAL@50 to 60Hz"));
                rto->presetIsPalForce60 = 1;
            }
            if (videoMode == VM_PAL_576i) {
                videoMode = VM_NTSC_480i;
            }
            if (videoMode == VM_PAL_576p) {
                videoMode = VM_NTSC_480p;
            }
        }
    }

//    if (videoModeIsNtsc(videoMode) || videoMode == VM_MED_RES_25kHz || videoMode == VM_NOT_RECOGNISED || videoMode == VM_VGA_UPSCALE) {
      if ((videoModeIsNtsc(videoMode) || videoMode == VM_MED_RES_25kHz || videoMode == VM_VGA_UPSCALE) && (videoMode != VM_NOT_RECOGNISED)) {
        if (uopt->presetPreference == PP_NORMAL) {
            writeProgramArrayNew(ntsc_240p, false);
        } else if (uopt->presetPreference == PP_720x480_768x576) {
            writeProgramArrayNew(ntsc_720x480, false);
        } else if (uopt->presetPreference == PP_1280x720) {
            writeProgramArrayNew(ntsc_1280x720, false);
        }
#if defined(ESP8266)
        else if (uopt->presetPreference == PP_CUSTOM) {
            const uint8_t *preset = loadPresetFromSPIFFS(videoMode);
            writeProgramArrayNew(preset, false);
        } else if (uopt->presetPreference == PP_1280x1024) {
            if (uopt->matchPresetSource && (videoMode != VM_MED_RES_25kHz) && (GBS::GBS_OPTION_SCALING_RGBHV::read() == 0)) {
...
#endif
        else if (uopt->presetPreference == PP_1920x1080) {
            writeProgramArrayNew(ntsc_1920x1080, false);
        } else if (uopt->presetPreference == PP_DOWNSCALE) {
            writeProgramArrayNew(ntsc_downscale, false);
        }
    }
    else if (videoModeIsPal(videoMode)) {
        // includes VM_PAL_324p
        if (uopt->presetPreference == PP_NORMAL) {
    ...
    else if (videoMode == VM_HD_720p || videoMode == VM_HD_1080i || videoMode == VM_HD_1080p || videoMode == VM_VGA_SVGA_XGA_SXGA) {
        // use bypass mode for these HD sources
        setOutModeHdBypass();
        return videoMode;
    }
    else if (videoMode == VM_RGBHV) {
    ...
    // get auto gain prefs
    if (uopt->presetPreference == PP_CUSTOM && uopt->enableAutoGain) {
...
// unrecognized input line count must remain stable for this no of checks before VM_UNKNOWN->VM_NOT_RECOGNISED
#define NOT_RECOGNISED_COUNT_MAX 255

#define VPERIOD_PAL_324p 324 // VPERIOD_IF actually reports double this (647)
#define VPERIOD_TOLERANCE 1 // used to detect 324 lines 
    ...
    if ((rto->videoStandardInput == VM_VGA_UPSCALE) || (rto->videoStandardInput == VM_RGBHV)) { // check RGBHV first // not mode 13 here, else mode 13 can't reliably exit
    ...
    if ((detectedMode & 0x07) == 0x07) {
        if ((detectedMode & 0x80) == 0x80) { // bit 7: SD flag (480i, 480P, 576i, 576P)
            if ((detectedMode & 0x08) == 0x08)
                return VM_NTSC_480i; // ntsc interlace
            if ((detectedMode & 0x20) == 0x20)
                return VM_PAL_576i; // pal interlace
            if ((detectedMode & 0x10) == 0x10)
                return VM_NTSC_480p; // 480p 60 progressive
            if ((detectedMode & 0x40) == 0x40)
                return VM_PAL_576p; // 576p 50 progressive
        }
        else {
            // stable but not SD mode
            // check for 324p for Marconi 2380
            // VPERIOD_IF can read double the expected value (reason unknown)
            uint16_t vperiod = GBS::VPERIOD_IF::read();
            if ( ((vperiod >= VPERIOD_PAL_324p - VPERIOD_TOLERANCE ) && (vperiod <= VPERIOD_PAL_324p + VPERIOD_TOLERANCE)) ||
                 ((vperiod >= (2 * VPERIOD_PAL_324p) - VPERIOD_TOLERANCE) && (vperiod <= (2 * VPERIOD_PAL_324p) + VPERIOD_TOLERANCE)) )
                return VM_PAL_324p;          
        }
        ...
        if (rto->videoStandardInput == VM_PAL_576p) {
            detectedMode = GBS::STATUS_04::read();
            if ((detectedMode & 0xFF) == 0x80) {
                return VM_PAL_576p; // still 576p 50 progressive
            }
        }
    }
    ...
    if ((detectedMode & 0x20) == 0x20) { // hd mode on
        ...
        if ((detectedMode & 0x10) == 0x10) {
            if ((detectedMode & 0x04) == 0x04) { // normally HD2376_1250P (PAL FHD?), but using this for 24k
                return VM_MED_RES_25kHz;
            }
            return VM_HD_1080p; // hdtv 1080p
        }
    }
    ...
    if ((GBS::STATUS_05::read() & 0x0c) == 0x00) // 2: Horizontal unstable AND 3: Vertical unstable are 0?
    {
        ...
        if (GBS::STATUS_00::read() == 0x07) {            // the 3 stat0 stable indicators on, none of the SD indicators on
            ...
            {
                if (rto->inputIsYpBpR)
                    return VM_VGA_SVGA_XGA_SXGA;
                else
                    return VM_RGBHV; // switch to RGBS/HV handling
            } else {
    ...
    if ((detectedMode & 0x2F) == 0x07) { // 0_00 H+V stable, not NTSCI, not PALI
        ...
        if ((detectedMode & 0x02) == 0x02) { // SP H active
            lineCount = GBS::STATUS_SYNC_PROC_VTOTAL::read();        
            ...
            if (lineCount != 0 && rto->notRecognizedCounter < NOT_RECOGNISED_COUNT_MAX) {
    ...
    if (rto->notRecognizedCounter == NOT_RECOGNISED_COUNT_MAX)
        return VM_NOT_RECOGNISED;       
    
    return VM_UNKNOWN; // unknown mode
}
...
{
    if (rto->videoStandardInput == VM_RGBHV) { // check RGBHV first
    ...
    if ((status16 & 0x02) == 0x02) {
        if (videoModeIsPalNtscSd(rto->videoStandardInput) || (rto->videoStandardInput == VM_PAL_324p)) {
...
void setOverSampleRatio(uint8_t newRatio, boolean prepareOnly)
{
    ...
    switch (newRatio) {
        case 1:
            ...
            if (rto->videoStandardInput == VM_MED_RES_25kHz || rto->videoStandardInput == VM_PAL_576p || rto->videoStandardInput == VM_NTSC_480p) {
...
void updateSpDynamic(boolean withCurrentVideoModeCheck)
{
    ...
    vm_t vidModeReadout = getVideoMode();
    if (vidModeReadout == VM_UNKNOWN) {
        vidModeReadout = getVideoMode();
    }

    if ((rto->videoStandardInput == VM_UNKNOWN) && (vidModeReadout == VM_UNKNOWN)) {
    ...
    if (rto->videoStandardInput != VM_UNKNOWN) {
        if (videoModeIsPalNtscSd(rto->videoStandardInput)) { // SD interlaced
            ...
            GBS::SP_H_PULSE_IGNOR::write(0x06);
                   ...
                   (rto->videoStandardInput == VM_VGA_UPSCALE) ||
                   (rto->videoStandardInput == VM_RGBHV) ) { // 13, 14 and 15 (was just 13 and 15)
            if (rto->syncTypeCsync == false) {
...
{
    if (((rto->videoStandardInput == VM_UNKNOWN) || (rto->videoStandardInput == VM_RGBHV)) ||
    ...
    if (accInHlength <= 240) {
        ...
        if (GBS::STATUS_SYNC_PROC_VTOTAL::read() <= 322) {
            ...
            if (GBS::STATUS_SYNC_PROC_VTOTAL::read() <= 322) {
                ...
                if (rto->syncTypeCsync && (rto->videoStandardInput != VM_UNKNOWN) &&
                   ( videoModeIsPalNtsc(rto->videoStandardInput))) {
...
{
    if ((rto->videoStandardInput == VM_UNKNOWN) || !rto->boardHasPower || rto->sourceDisconnected) {
        return;
    }
...
void setOutModeHdBypass()
{
    ...
    GBS::PLL648_CONTROL_01::write(PLL648_CONTROL_01_ADCCLK);
    ...
    // Unsure if (rto->videoStandardInput == VM_UNKNOWN) (i.e 0) should be included here but original code was 'if (rto->videoStandardInput <=2)'
    if ((rto->videoStandardInput == VM_UNKNOWN) || videoModeIsPalNtscSd(rto->videoStandardInput)) {
        // The following code is only executed for PAL & NTSC interlaced modes (1 & 2).
        //
        ...
        if (rto->videoStandardInput == VM_PAL_576i) {
    ...
    } else if ((rto->videoStandardInput == VM_NTSC_480p) || (rto->videoStandardInput == VM_PAL_576p)) { // 480p, 576p
        ...
        if (rto->videoStandardInput == VM_NTSC_480p) {
    ...
    } else if ((rto->videoStandardInput <= VM_HD_1080i) || (rto->videoStandardInput == VM_VGA_SVGA_XGA_SXGA)) {
        //GBS::SP_HS2PLL_INV_REG::write(0); // 5_56 1 use rising edge of tri-level sync // always 0 now
        if (rto->videoStandardInput == VM_HD_720p) { // 720p
        ...
        if (rto->videoStandardInput == VM_HD_1080i) { // 1080i
            // interl. source
        ...
        if (rto->videoStandardInput == VM_HD_1080p) {  // 1080p
        ...
        if (rto->videoStandardInput == VM_VGA_SVGA_XGA_SXGA) { // odd HD mode (PS2 "VGA" over Component)
    ...
    if (rto->videoStandardInput == VM_VGA_SVGA_XGA_SXGA) {
    ...
    rto->presetID = PID_BYPASS_1; // bypass flavor 1, used to signal buttons in web ui
    GBS::GBS_PRESET_ID::write(PID_BYPASS_1);
    ...
    updateSpDynamic(0);
    while ((getVideoMode() == VM_UNKNOWN) && (millis() - timeout < 1502)) {
        delay(1);
    }
...
void bypassModeSwitch_RGBHV()
{
    ...
    applyRGBPatches();
    resetDebugPort();
    rto->videoStandardInput = VM_RGBHV;       // making sure
    ...
    rto->presetID = PID_BYPASS_2; // bypass flavor 2, used to signal buttons in web ui
    GBS::GBS_PRESET_ID::write(PID_BYPASS_2);
    delay(200);
}
...
boolean snapToIntegralFrameRate(void)
{
    ...
    SerialM.print(target, 1); // precision 1
    SerialM.println("Hz");
...
void printInfo()
{
    ...
    sprintf(print, "h:%4u v:%4u PLL:%01u A:%02x%02x%02x S:%02x.%02x.%02x %c%c%c%c I:%02x D:%04x m:%hu ht:%4d vt:%4d hpw:%4d u:%3x s:%2x S:%2d W:%2d\n",
            ...
            h, HSp, v, VSp, stat0FIrq, GBS::TEST_BUS::read(), (uint8_t)getVideoMode(),
...
void runSyncWatcher()
{
    ...
    vm_t detectedVideoMode = getVideoMode();
    ...
    if (rto->videoStandardInput == VM_VGA_SVGA_XGA_SXGA) { // using flaky graphic modes
        if (detectedVideoMode == VM_UNKNOWN) {
            if (GBS::STATUS_INT_SOG_BAD::read() == 0) {
                detectedVideoMode = VM_VGA_SVGA_XGA_SXGA; // then keep it
            }
        }
    }
    ...
    if (((detectedVideoMode == VM_UNKNOWN) || !status16SpHsStable) && (rto->videoStandardInput != VM_RGBHV)) {
        rto->noSyncCounter++;
        rto->continousStableCounter = 0;
        lastVsyncLock = millis(); // best reset this
        ...
        if (newVideoModeCounter == 0) {
            ...
            if (rto->noSyncCounter == 2) { // this usually repeats
                ...
                if (rto->currentLevelSOG <= 1 && videoModeIsPalNtscSd(rto->videoStandardInput)) {
                    rto->currentLevelSOG += 1;
                    setAndUpdateSogLevel(rto->currentLevelSOG);
                    delay(30);
                }
    ...
    if (((detectedVideoMode != VM_UNKNOWN && detectedVideoMode != rto->videoStandardInput) ||
         (detectedVideoMode != VM_UNKNOWN && rto->videoStandardInput == VM_UNKNOWN)) &&
        rto->videoStandardInput != VM_RGBHV) {
        ...
        if (newVideoModeCounter >= 8) {
            vm_t vidModeReadout = VM_UNKNOWN;
            SerialM.print(F("\nFormat change:"));
            for (int a = 0; a < 30; a++) {
                vidModeReadout = getVideoMode();
                if (vidModeReadout == VM_VGA_SVGA_XGA_SXGA) {
            ...
            if (newVideoModeCounter != 0) {
                ...
                //Serial.print("Old: "); Serial.print((uint8_t)rto->videoStandardInput);
                //Serial.print(" New: "); Serial.println((uint8_t)detectedVideoMode);
                rto->videoIsFrozen = false;
                ...
                boolean wantPassThroughMode = uopt->presetPreference == PP_720x480_768x576;

                if ((videoModeIsNtsc(rto->videoStandardInput) && videoModeIsPal(detectedVideoMode)) ||
                    (rto->videoStandardInput == VM_UNKNOWN) ||
                    (videoModeIsPal(rto->videoStandardInput) && videoModeIsNtsc(detectedVideoMode)) ) {
                ...
                if (!wantPassThroughMode) {
                    ...
                    rto->videoStandardInput = applyPresets(detectedVideoMode);
                } else {
    ...
    } else if (getStatus16SpHsStable() && (detectedVideoMode != VM_UNKNOWN) && (rto->videoStandardInput != VM_RGBHV) && (rto->videoStandardInput == detectedVideoMode)) {
    ...
    if ((rto->videoStandardInput == VM_VGA_UPSCALE) || (rto->videoStandardInput == VM_RGBHV)) { // RGBHV checks
        ...
        if (uopt->preferScalingRgbhv && rto->continousStableCounter >= 2) {
            ...
            // is the source in range for scaling RGBHV and is it currently in mode VM_RGBHV (15)?
            uint16 sourceLines = GBS::STATUS_SYNC_PROC_VTOTAL::read(); // if sourceLines = 0, might be in some reset state
            if ((sourceLines <= 535 && sourceLines != 0) && rto->videoStandardInput == VM_RGBHV) {
                ...
                if (moveOn) {
                    ...
                    vm_t videoMode = rto->videoStandardInput;
                    // todo: this hack is hard to understand when looking at applypreset and mode is suddenly 1,2 or 3
                    if (uopt->presetPreference == PP_CUSTOM) {
                        // custom preset defined, try to load (set mode = VM_VGA_UPSCALE here early)
                        videoMode = VM_VGA_UPSCALE;
                    } else {
                        if (sourceLines < 280) {
                            ...
                            videoMode = VM_PAL_576i;
                        } else if (sourceRate > 44.0f && sourceRate < 53.8f) {
                            // not low res but PAL = "50 Hz EDTV"
                            videoMode = VM_PAL_576p;
                            needPostAdjust = 1;
                    ...
                    if (uopt->presetPreference == PP_720x480_768x576) {
                        uopt->presetPreference = PP_NORMAL; // fix presetPreference which can be "bypass"
                    }

                    activePresetLineCount = sourceLines;
                    rto->videoStandardInput = applyPresets(videoMode);
                    ...
                    if (rto->extClockGenDetected) {
                        // switch to ext clock
                        if (!rto->outModeHdBypass) {
                            if (GBS::PLL648_CONTROL_01::read() != PLL648_CONTROL_01_ADCCLK && GBS::PLL648_CONTROL_01::read() != PLL648_CONTROL_01_EXTCLK) {
                                ...
                                GBS::PLL648_CONTROL_01::write(PLL648_CONTROL_01_EXTCLK);
                            }
                        }
                        // sync clocks now
                        externalClockGenSyncInOutRate();
            ...
            // if currently in scaling RGB/HV, check for "SD" < > "EDTV" style source changes
            else if ((sourceLines <= 535 && sourceLines != 0) && (rto->videoStandardInput == VM_VGA_SVGA_XGA_SXGA)) {
                // todo: custom presets?
                if (sourceLines < 280 && activePresetLineCount > 280) {
                    rto->videoStandardInput = VM_NTSC_480i;
                } else if (sourceLines < 380 && activePresetLineCount > 380) {
                    rto->videoStandardInput = VM_PAL_576i;
                } else if (sourceLines > 380 && activePresetLineCount < 380) {
                    rto->videoStandardInput = VM_NTSC_480p;
                    needPostAdjust = 1;
                }
                ...
                if (rto->videoStandardInput != VM_VGA_UPSCALE) {
                    ...
                    if (moveOn) {
                        ...
                        activePresetLineCount = sourceLines;
                        rto->videoStandardInput = applyPresets(rto->videoStandardInput);
            ...
            // check whether to revert back to full bypass
            else if ((sourceLines > 535) && rto->videoStandardInput == VM_VGA_UPSCALE) {
                uint16_t firstDetectedSourceLines = sourceLines;
                boolean moveOn = 1;
                for (int i = 0; i < 30; i++) {
                    sourceLines = GBS::STATUS_SYNC_PROC_VTOTAL::read();
                ...
                if (moveOn) {
                    ...
                    rto->videoStandardInput = applyPresets(VM_RGBHV); // exception: apply preset here, not later in syncwatcher
        ...
        if (!uopt->preferScalingRgbhv && rto->videoStandardInput == VM_VGA_UPSCALE) {
...
{
    uopt->presetPreference = PP_NORMAL;
...
void setup()
{
    ...
    if (!SPIFFS.begin()) {
        ...
        if (!f) {
            ...
            uopt->presetPreference = (preset_t)((uint8_t)(f.read() - '0')); // #1
            if (uopt->presetPreference > PP_BYPASS)
                uopt->presetPreference = PP_NORMAL; // fresh spiffs ?
...
void loop()
{
    ...
    if (typeOneCommand != '@') {
        ...
        switch (typeOneCommand) {
            ...
            case 'K':
                setOutModeHdBypass();
                uopt->presetPreference = PP_BYPASS;
                saveUserPrefs();
                break;
            case 'T':
            ...
            case 'b':
                ...
                break;
            case '#':
                rto->videoStandardInput = applyPresets(VM_VGA_SVGA_XGA_SXGA);
      ...
      updateCoastPosition();*/
            ...
            case 'L': {
                ...
                preset_t backup = uopt->presetPreference;
                uopt->presetPreference = PP_1280x1024;
                rto->videoStandardInput = applyPresets(VM_UNKNOWN); // force hard reset
            ...
            case 'Z': {
                ...
                saveUserPrefs();
                vm_t vidMode = getVideoMode();
                if (uopt->presetPreference == PP_NORMAL && GBS::GBS_PRESET_ID::read() == PID_1280x960_PAL) {
                    rto->videoStandardInput = applyPresets(vidMode);
                } else if (uopt->presetPreference == PP_1280x1024 && GBS::GBS_PRESET_ID::read() == PID_1280x1024_NTSC) {
                    rto->videoStandardInput = applyPresets(vidMode);
                }
            } break;
            case 'W':
    ...
    if (rto->sourceDisconnected == false && rto->syncWatcherEnabled == true && (millis() - lastTimeSyncWatcher) > 20) {
        ...
        // auto adc gain
        if (uopt->enableAutoGain == 1 && !rto->sourceDisconnected && rto->videoStandardInput != VM_UNKNOWN && rto->clampPositionIsSet && rto->noSyncCounter == 0 && rto->continousStableCounter > 90 && rto->boardHasPower) {
            uint16_t htotal = GBS::STATUS_SYNC_PROC_HTOTAL::read();
            uint16_t pllad = GBS::PLLAD_MD::read();
            if (((htotal > (pllad - 3)) && (htotal < (pllad + 3)))) {
    ...
    if (((rto->videoStandardInput != VM_RGBHV) && (rto->videoStandardInput != VM_UNKNOWN)) &&
    ...
    if ((rto->videoStandardInput != VM_UNKNOWN) && (rto->continousStableCounter >= 4) &&
         ...
         !rto->syncWatcherEnabled))                                                // or that
    {
        if (rto->applyPresetDoneStage == 1) {
            ...
            if (rto->extClockGenDetected && rto->videoStandardInput != VM_VGA_UPSCALE) {
                // switch to ext clock
                if (!rto->outModeHdBypass) {
                    if (GBS::PLL648_CONTROL_01::read() != PLL648_CONTROL_01_ADCCLK && GBS::PLL648_CONTROL_01::read() != PLL648_CONTROL_01_EXTCLK) {
                        // first store original in an option byte in 1_2D
    ...
    if (rto->syncWatcherEnabled == true && rto->sourceDisconnected == true && rto->boardHasPower) {
        if ((millis() - lastTimeSourceCheck) >= 500) {
            if (checkBoardPower()) {
                (void)inputAndSyncDetect(); // source is off or just started; keep looking for new input
            } else {
                rto->boardHasPower = false;
                rto->continousStableCounter = 0;
                rto->syncWatcherEnabled = false;
...
//****************************************
// web user interface command handlng
void handleType2Command(char argument)
{
    switch (argument) {
        ...
        {
            uopt->presetPreference = PP_CUSTOM;
            if (rto->videoStandardInput == VM_VGA_UPSCALE) {
        ...
        case 'L': {
            ...
            vm_t videoMode = getVideoMode();
            if (videoMode == VM_UNKNOWN && GBS::STATUS_SYNC_PROC_HSACT::read())
                videoMode = rto->videoStandardInput; // last known good as fallback
            //else videoMode stays 0 and we'll apply via some assumptions

            if (argument == 'f')
                uopt->presetPreference = PP_CUSTOM; // 1280x960
            if (argument == 'g')
                uopt->presetPreference = PP_1280x720; // 1280x720
            if (argument == 'h')
                uopt->presetPreference = PP_720x480_768x576; // 720x480/768x576
            if (argument == 'p')
                uopt->presetPreference = PP_1280x1024; // 1280x1024
            if (argument == 's')
                uopt->presetPreference = PP_1920x1080; // 1920x1080
            if (argument == 'L')
                uopt->presetPreference = PP_DOWNSCALE; // 15kHz downscale (480i/480p) to 240p
        ...
        case 'v': {
            ...
            saveUserPrefs();
            vm_t vidMode = getVideoMode();
            if (uopt->presetPreference == PP_1920x1080) {
                if (GBS::GBS_PRESET_ID::read() == PID_1920x1080_NTSC || GBS::GBS_PRESET_ID::read() == PID_1920x1080_PAL) {
                    rto->videoStandardInput = applyPresets(vidMode);
                }
            }
        } break;
...
static const uint8_t *loadPresetFromSPIFFS(vm_t forVideoMode)
{
    ...
    if (f) {
        ...
        if (videoModeIsPal(forVideoMode))
            return pal_240p;
        else
            return ntsc_240p;
    }
...
void savePresetToSPIFFS()
{
    ...
    if (rto->videoStandardInput == VM_PAL_324p) {
        f = SPIFFS.open("/preset_pal_324p." + String((char)slot), "w");
    } else if (rto->videoStandardInput == VM_NTSC_480i) {
...
void saveUserPrefs()
{
    ...
    f.write((uint8_t)(uopt->presetPreference + '0')); // #1
...
static void resolution_OLED( vm_t videoMode, preset_t presetPreference, const String& s)
{
  ...
  if (presetPreference == PP_BYPASS) {
    ...
    if (videoMode == VM_UNKNOWN && GBS::STATUS_SYNC_PROC_HSACT::read())
        videoMode = rto->videoStandardInput;
    uopt->presetPreference = presetPreference;
    rto->useHdmiSyncFix = 1;
    if (rto->videoStandardInput == VM_VGA_UPSCALE) {
      rto->videoStandardInput = VM_RGBHV;
    } else {
...
//****************************************
static void presets_OLED(uint8_t slot, uint8_t preset)
{
    uopt->presetSlot = slot;
    uopt->presetPreference = PP_CUSTOM;
    saveUserPrefs();
    for (int i = 0; i <= 280; i++) {
        display.clear();
        display.setFont(Open_Sans_Regular_20);
        display.drawString(64, -8, "Preset #" + String(oled_Presets[preset]));
...
static void settingsMenuOLED(void)
{
    ...
    //1280x960
    if (oled_menuItem == 1) {
      if (oled_selectOption == 2) {
        //1280x960       
        if (oled_pointer_count == 0) {
            oled_subsetFrame = 3;
            resolution_OLED(videoMode, PP_NORMAL, "1280x960");
            oled_selectOption = 1;
            oled_subsetFrame = 1;
        }
        //1280x1024
        if (oled_pointer_count == 1) {
            oled_subsetFrame = 3;
            resolution_OLED(videoMode, PP_1280x1024, "1280x1024");
            oled_selectOption = 1;
            oled_subsetFrame = 1;
        }
        //1280x720
        if (oled_pointer_count == 2) {
            oled_subsetFrame = 3;
            resolution_OLED(videoMode, PP_1280x720, "1280x720");
            oled_selectOption = 1;
            oled_subsetFrame = 1;
        }
        //1920x1080
        if (oled_pointer_count == 3) {
            oled_subsetFrame = 3;
            resolution_OLED(videoMode, PP_1920x1080, "1920x1080");          
            oled_selectOption = 1;
            oled_subsetFrame = 1;
        }
        //720x480
        if (oled_pointer_count == 4) {
            oled_subsetFrame = 3;
            resolution_OLED(videoMode, PP_720x480_768x576, "480/576");
            oled_selectOption = 1;
            oled_subsetFrame = 2;
        }
        //downscale
        if (oled_pointer_count == 5) {
            oled_subsetFrame = 3;
            resolution_OLED(videoMode, PP_DOWNSCALE, "Downscale");
            oled_selectOption = 1;
            oled_subsetFrame = 2;
        }
        //passthrough/bypass
        if (oled_pointer_count == 6) {
            oled_subsetFrame = 3;
            resolution_OLED(videoMode, PP_BYPASS, "Pass-Through");
            oled_selectOption = 1;
            oled_subsetFrame = 2;
        }
        //go back
        if (oled_pointer_count == 7) {
            oled_page = 0;
            oled_subsetFrame = 0;
            oled_main_pointer = 0;
            oled_sub_pointer = 0;
    ...
    //Preset selection
    if (oled_menuItem == 2 && oled_selectOption == 2) {
        if (oled_pointer_count == 0) {
            oled_subsetFrame = 3;
            presets_OLED('A', 0);
        ...
        if (oled_pointer_count == 4) {
            ...
            presets_OLED('E', 4);
            oled_selectOption = 1; //reset select container
            oled_subsetFrame = 2;  //switch back to prev frame (prev screen)
        }
        if (oled_pointer_count == 5) {
            oled_subsetFrame = 3;
            presets_OLED('F', 5);
    ...
    if (oled_menuItem == 4 && oled_subsetFrame == 1) {
        ...
        if (rto->presetID == PID_1280x960_NTSC || rto->presetID == PID_1280x960_PAL) {
